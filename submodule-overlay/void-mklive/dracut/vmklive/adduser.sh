...
echo lyargoos-live > ${NEWROOT}/etc/hostname
...
# Setup default root/user password (lyargoos).
chroot ${NEWROOT} sh -c 'echo "root:lyargoos" | chpasswd -c SHA512'
chroot ${NEWROOT} sh -c "echo "$USERNAME:lyargoos" | chpasswd -c SHA512"

# Enable sudo permission by default.
if [ -f ${NEWROOT}/etc/sudoers ]; then
    echo "${USERNAME} ALL=(ALL:ALL) NOPASSWD: ALL" > "${NEWROOT}/etc/sudoers.d/99-lyargoos-live"
fi

if [ -d ${NEWROOT}/etc/polkit-1 ]; then
    # If polkit is installed allow users in the wheel group to run anything.
    cat > ${NEWROOT}/etc/polkit-1/rules.d/lyargoos-live.rules <<_EOF
...
_EOF
    chroot ${NEWROOT} chown polkitd:polkitd /etc/polkit-1/rules.d/lyargoos-live.rules
fi
